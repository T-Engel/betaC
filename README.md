...
beta-diversity can be traced back to two types of sampling effects. The
first one arises because alpha and gamma scale diversity estimates are
usually based on very different sample sizes (i.e., numbers of
individuals captured by a sample). The second concerns sample
completeness and the fact that sample size is always relative. In small
species pools, relatively small sample sizes are sufficient to get a
pretty complete sample, while in large species pools sample sizes have
to be increased dramatically to achieve the same degree of sample
completeness (e.g. capturing 80% of all the species in the area).
...
The example below will hopefully help drive home this argument.
...
the sample size *n* used for rarefaction is called a individual based
rarefaction curve. The r package `vegan` provides functions to carry out
IBR. Let’s plot the IBR curve for gamma scale and see how the species
richness changes when we rarefy it down to the sample size of the alpha
scale.
...
more individuals than the alpha scale. Isn’t that an unfair comparison?
Let’s look at the alpha scale to see how it compares.
...
interested in the average plot. Therefore let’s take the mean of these
curves and plot in the same graph as the gamma scale
...
plot(gamma_curve, xlab = "Number of individuals", ylab= "Rarefied richness", type = "l", lwd=2)
abline(v = N_alpha, col="grey", lty=3)
