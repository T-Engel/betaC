...
In our paper we make the argument that the species-pool dependence of beta-diversity can be traced back to two types of sampling effects. The first one arises because alpha and gamma scale diversity estimates are usually based on very different sample sizes (i.e.,  numbers of individuals captured by a sample). The second concerns sample completeness and the fact that sample size is always relative. In small species pools, relatively small sample sizes are sufficient to get a pretty complete sample, while in large species pools sample sizes have to be increased dramatically to achieve the same degree of sample completeness (e.g. capturing 80% of all the species in the area). 
...
The grey vertical line marks the alpha scale sample size and the horizontal grey line is the rarefied richness of the gamma scale that corresponds to this sample size. It looks like about half of the species on the gamma scale have only been found because the gamma scale as way more individuals than the alpha scale. Isn't that an unfair comparison? Let's look at the alpha scale to see how it compares.
...
These are all the alpha scale rarefaction curves. We are usually interested in the average plot. Therefore let's take the mean of these curves and plot in the same graph as the gamma scale
